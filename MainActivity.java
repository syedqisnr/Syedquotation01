...
public class MainActivity extends BridgeActivity {
                    ...
                    try {
                        ...
                        if (acceptTypes != null && acceptTypes.length > 0) {
                            ...
                            for (String type : acceptTypes) {
                                ...
                                if (clean.equals(".pdf") || clean.contains("pdf")) {
                                    mimeList.add("application/pdf");
                                } else if (clean.equals(".txt") || clean.contains("plain") || clean.equals("text/*")) {
                                    mimeList.add("text/plain");
                                } else if (clean.equals(".doc") || clean.contains("msword")) {
                                    mimeList.add("application/msword");
                                } else if (clean.equals(".docx") || clean.contains("wordprocessingml")) {
                                    mimeList.add("application/vnd.openxmlformats-officedocument.wordprocessingml.document");
                                } else if (clean.contains("/")) {
                                    mimeList.add(clean);
                                }
                            }
                            if (!mimeList.isEmpty()) {
                                chooserIntent.setType(mimeList.get(0));
                                if (mimeList.size() > 1) {
                                    chooserIntent.putExtra(Intent.EXTRA_MIME_TYPES, mimeList.toArray(new String[0]));
                                }
                            } else {
    ...
    public class AndroidBridge {
        ...
        public String getVersion() {
            return "22.1.0 (135 Tools + Turnkey Engine)";
        }
    ...
    private void decodeImageToJpegDirect(String base64Data, String requestId) {
        try {
            ...
            if (bitmap == null) {
                notifyJs("window.__onNativeImageDecodeResult && window.__onNativeImageDecodeResult(" + jsStringLiteral(requestId) + ", null, " + jsStringLiteral("Device could not decode this image.") + ")");
                return;
            }
