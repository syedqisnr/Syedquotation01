    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        try {
            ...
            webView.setWebChromeClient(new WebChromeClient() {
                ...
                // Android's default WebView silently swallows alert()/confirm()/prompt() calls
                // unless a WebChromeClient explicitly handles them — no dialog appears at all,
                // and prompt() just returns null instantly. That's why the "Enter Security PIN"
                // button (and every other validation alert() in the app) looked like it did
                // nothing when tapped. These three overrides make real native dialogs appear.
                @Override
                ...
                public boolean onJsPrompt(WebView view, String url, String message, String defaultValue, final android.webkit.JsPromptResult result) {
                    try {
                        ...
                        boolean looksLikePin = message != null && message.toLowerCase().contains("pin");
                        if (looksLikePin) {
                            input.setInputType(android.text.InputType.TYPE_CLASS_NUMBER | android.text.InputType.TYPE_NUMBER_VARIATION_PASSWORD);
                            input.setTransformationMethod(android.text.method.PasswordTransformationMethod.getInstance());
                        }
                        if (defaultValue != null && !defaultValue.equals("null")) input.setText(defaultValue);
